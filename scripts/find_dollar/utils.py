...
def calculate_timestamp(time_delta_string):
    ...
    time_delta = datetime.datetimedelta(days=days, hours=hours, minutes=minutes, seconds=seconds)
...
def print_results(today_rate, five_days_ago_diff, first_day_diff, one_month_ago_diff, dates):
    ...
    day_col_width = max(len("Day"), len(today.strftime('%b %d')), len(dates[0].strftime('%b %d')), len(dates[1].strftime('%b %d')), len(dates[2].strftime('%b %d')))
    ...
    print("| {} | {} |".format(dates[0].strftime('%b %d').ljust(day_col_width), "{:.2f}%".format(five_days_ago_diff).rjust(rate_col_width)))
    print("| {} | {} |".format(dates[1].strftime('%b %d').ljust(day_col_width), "{:.2f}%".format(first_day_diff).rjust(rate_col_width)))
    print("| {} | {} |".format(dates[2].strftime('%b %d').ljust(day_col_width), "{:.2f}%".format(one_month_ago_diff).rjust(rate_col_width)))
